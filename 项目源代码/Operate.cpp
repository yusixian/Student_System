	...
	while (flag) {
		...
		for (it = ss.begin(); it != ss.end(); it++) {
			if (strcmp(stunum, (*it).stuNum) == 0) {
				...
				(*it).display();
				cout << endl;
				cout << "\t\t\t---确认要修改吗？按ESC可退出，按其他任意键可进入下一步---"  << endl;
				int c = _getch();
				if (c == 27) {	// ESC
					break;
				}
...
void Operate::Rank_Update(Student& S) {
	vector<Student> ss = ReadStudentFile();
	vector<Student>::iterator it;
	int averank = 1;
	int mulrank = 1;
	for (it = ss.begin(); it != ss.end(); it++) {
		if (strcmp(S.Class, (*it).Class) == 0 &&
			strcmp(S.stuNum, (*it).stuNum) != 0) {	//保证是同班同学
			if (S.avrGrade < (*it).avrGrade)
				averank++;
			if (S.multiGrade < (*it).multiGrade)
				mulrank++;
		}
	}
	S.examRank = averank;
	S.multiGradeRank = mulrank;
	for (it = ss.begin(); it != ss.end(); it++) {
		if (strcmp(S.Class, (*it).Class) == 0) {
			if ((*it).examRank >= averank)	//原本排名在其之后的都下降一名
				(*it).examRank++;
			if ((*it).multiGradeRank >= mulrank)
				(*it).multiGradeRank++;
		}
	}
	ss.push_back(S);
